...
There are many encampements across the map that may aid in this endevour, allowing you to fortify positions and take map control.
...
### SOLDIER
SOLDIERs form the core of your army. Use them to fight your way to victory. They autoattack all units around them and can also capture encampements to give their army a strategic bonus.


Encampements
------------

In addition to the two basic units, there are special units called ENCAMPEMENTS that can be created on top of unoccupied encampement squares.

If a soldier is located on an encampement square, it has the option to capture the square (after `GameConstants.CAPTURE_DELAY`) and turn it into one of the following encampements:
...
Power is shared globally across the team and is generated by the HQ at 40 power per round (`GameConstants.HQ_RESOURCE_PRODUCTION`).  4 Additional power can be generated by GENERATOR encampements (`GENERATOR_RESOURCE_PRODUCTION`).
...
- The map will have neutral encampements scattered across the map. There will be a minimum of `5` (`GameConstants.MAP_MINIMUM_ENCAMPMENTS`) encampements. There will be no encampements orthogonally adjacent to your base
