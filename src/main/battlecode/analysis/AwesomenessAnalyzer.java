...
//				} else if(signal instanceof EvolutionSignal) {
//					EvolutionSignal s = (EvolutionSignal)signal;
//					r = robots.get(s.getRobotID());
//					r.resetCountDown();
//					r.transform(s.getType());
//					events.add(new Event(EVOLVE_AWESOMENESS*r.robotAwesomeness, robots.get(s.getRobotID()).location));
...
//					float awesomeness = r.type==RobotType.ARCHON?ARCHON_DEATH_AWESOMENESS:DEATH_AWESOMENESS*r.robotAwesomeness;
//					events.add(new Event(awesomeness, r.location));
//				} else if(signal instanceof ConvexHullSignal) {
//					ConvexHullSignal s = (ConvexHullSignal)signal;
//					ConvexHullData newData = new ConvexHullData(s.getConvexHulls());
//					ConvexHullData oldData = hulls[s.getTeam().ordinal()];
//					float areaDiff=(newData.area_2-oldData.area_2)/2.f;
//					if(areaDiff!=0.) {
//						int x = (int)((newData.sum_X_6-oldData.sum_X_6)/(6.f*areaDiff));
//						int y = (int)((newData.sum_Y_6-oldData.sum_Y_6)/(6.f*areaDiff));
//						events.add(new Event(CONVEX_HULL_AWESOMENESS_PER_AREA*Math.abs(areaDiff),new MapLocation(x,y)));
//					}
//					hulls[s.getTeam().ordinal()]=newData;
//				}
//			}
//
