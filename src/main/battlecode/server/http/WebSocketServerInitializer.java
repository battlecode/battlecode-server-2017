...
public class WebSocketServerInitializer extends ChannelInitializer<SocketChannel> {
    ...
    private final PassthroughController controller;

    public WebSocketServerInitializer(final SslContext sslCtx,
            final ChannelGroup activeWebSockets,
            final WebSocketProxy proxy,
            final PassthroughController controller) {
        this.sslCtx = sslCtx;
    ...
    public void initChannel(SocketChannel ch) throws Exception {
        ...
        pipeline.addLast(new JacksonJsonCodec());
        pipeline.addLast(new CommandAdapter(controller));

        activeWebSockets.add(ch);
    }
}
