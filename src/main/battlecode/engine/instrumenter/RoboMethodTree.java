...
public class RoboMethodTree extends MethodNode implements Opcodes {
    ...
    private final String methodDesc; // the description of this method, e.g.,
                                     // "()V"
    private boolean codeVisited = false; // tells whether visitCode() has been
                                         // called
    ...
    public RoboMethodTree(final MethodVisitor mv, final String className, final int access, final String methodName,
            final String methodDesc, final String signature, final String[] exceptions, final String teamPackageName,
            final boolean debugMethodsEnabled, boolean silenced, boolean checkDisallowed) {
    ...
    private static AbstractInsnNode nextInstruction(AbstractInsnNode n) {
        while (n.getType() == AbstractInsnNode.LINE || n.getType() == AbstractInsnNode.FRAME
                || n.getType() == AbstractInsnNode.LABEL)
            n = n.getNext();
        return n;
    }

    @SuppressWarnings("unchecked") // This is to fix the warning from the add()
                                   // to tryCatchBlocks
    private void addRobotDeathHandler() {
        ...
        tryCatchBlocks.add(0, new TryCatchBlockNode(firstTryCatch, robotDeathLabel, robotDeathLabel,
    ...
    private void addDebugHandler() {
        ...
        instructions.insertBefore(nextInstruction(instructions.getFirst()), new MethodInsnNode(INVOKESTATIC,
                "battlecode/engine/instrumenter/RobotMonitor", "incrementDebugLevel", "()V", false));
        instructions.add(debugEndLabel);
        instructions.add(new FrameNode(F_FULL, 0, new Object[0], 1, new Object[] { "java/lang/Throwable" }));
        instructions.add(new MethodInsnNode(INVOKESTATIC, "battlecode/engine/instrumenter/RobotMonitor",
                "decrementDebugLevel", "()V", false));
        instructions.add(new InsnNode(ATHROW));
    }
    ...
    private void visitInsnNode(InsnNode n) {
        ...
        case RETURN:
            ...
            if (methodName.startsWith("debug_") && methodDesc.endsWith("V")) {
                instructions.insertBefore(n, new MethodInsnNode(INVOKESTATIC,
                        "battlecode/engine/instrumenter/RobotMonitor", "decrementDebugLevel", "()V", false));
            }
            break;
        case ATHROW:
            endOfBasicBlock(n);
        ...
        case MONITOREXIT:
            ...
            // We need to strip these so we don't leave monitors locked when a
            // robot dies
    ...
    private void visitMethodInsnNode(MethodInsnNode n) {
        ...
        if (n.getOpcode() == INVOKE_DYNAMIC_INSN) {
            // TODO: Handle Invoke Dynamic correctly to add support for
            // jruby/jython/etc. Checking submissions on CS for any
            // disqualifications from this.
            // throw new RuntimeException("Invoke Dynamic probably not
            // instrumented correctly by Battlecode Engine. Plz don't use for
            // now");
        }
        ...
        if (n.name.equals("hashCode") && n.desc.equals("()I") && n.getOpcode() != INVOKESTATIC) {
            ...
            instructions.insertBefore(n,
                    new MethodInsnNode(n.getOpcode(), classReference(n.owner), "hashCode", "()I", n.itf));
            ...
            if (n.getOpcode() == INVOKESPECIAL) {
                ...
                instructions.insertBefore(n, new InsnNode(DUP));
                instructions.insertBefore(n, new MethodInsnNode(INVOKEVIRTUAL, "java/lang/Object", "getClass",
        ...
        if (n.owner.equals("java/util/Random") && n.name.equals("<init>") && n.desc.equals("()V")) {
            instructions.insertBefore(n, new MethodInsnNode(INVOKESTATIC,
                    "battlecode/engine/instrumenter/lang/RoboRandom", "getMapSeed", "()J", false));
        ...
        if (checkDisallowed) {
            ...
            if ((n.desc.equals("()V")
                    && (n.name.equals("wait") || n.name.equals("notify") || n.name.equals("notifyAll")))
            ...
            if (n.owner.equals("java/io/PrintStream") && n.name.equals("<init>")
                    && n.desc.startsWith("(Ljava/lang/String;")) {
        ...
        boolean isDebugMethod = n.name.startsWith("debug_") && n.desc.endsWith("V")
                && n.owner.startsWith(teamPackageName);
        boolean endBasicBlock = n.owner.startsWith(teamPackageName)
                || classReference(n.owner).startsWith("instrumented") || n.owner.startsWith("battlecode");
        ...
        // But in practice this should be good enough.
        else if (n.name.equals("printStackTrace") && n.desc.equals("()V") && (n.owner == null
                || n.owner.equals("java/lang/Throwable") || isSuperClass(n.owner, "java/lang/Throwable"))) {
            instructions.insertBefore(n, new FieldInsnNode(GETSTATIC, "battlecode/engine/instrumenter/lang/System",
            ...
            // debug methods
            /*
             * if(name.startsWith("debug_")) { System.out.println("debug "
             * +className+" "+methodName+" "+owner+" "+name+" "
             * +checkDisallowed); }
             */
            if (isDebugMethod) {
                if (debugMethodsEnabled) {
                    // we need to end the basic block BEFORE the debug level is
                    // incremented
                    // endOfBasicBlock(n);
                    // instructions.insertBefore(n, new
                    // MethodInsnNode(INVOKESTATIC,
                    // "battlecode/engine/instrumenter/RobotMonitor",
                    // "incrementDebugLevel", "()V"));
                    // don't call endOfBasicBlock twice
                    // endBasicBlock=false;
                } else {
                    // if debug methods aren't enabled, we remove the call to
                    // the debug method
                    // first, pop the arguments from the stack
                    // arguments are popped in reverse order, so add
                    // instructions to newInsns
                    // using insert, not add
                    InsnList newInsns = new InsnList();
                    for (Type t : Type.getArgumentTypes(n.desc)) {
                        switch (t.getSize()) {
                        case 1:
                            // System.out.println("pop "+className+"
                            // "+methodName);
                    ...
                    // if we remove the method call and don't add any
                    // instructions then we could end up with a FrameNode that
                    // does not have an instruction following it. asm seems not
                    // to like this.
    ...
    private void illegalMethod(MethodInsnNode n, String message) {
        if (InstrumentingClassLoader.lazy()) {
            ...
            instructions.insertBefore(n,
                    new MethodInsnNode(INVOKESTATIC, "battlecode/engine/instrumenter/RoboMethodTree",
                            "reportIllegalMethod", "(Ljava/lang/String;)V", false));
        } else {
    ...
    @SuppressWarnings("unchecked") // n.local and n.stack are both supposed to
                                   // be List<Object>, but they aren't for some
                                   // reason?
    ...
    private void endOfBasicBlock(AbstractInsnNode n) {
        ...
        instructions.insertBefore(n, new MethodInsnNode(INVOKESTATIC, "battlecode/engine/instrumenter/RobotMonitor",
                "incrementBytecodes", "(I)V", false));
        bytecodeCtr = 0;
    }

    /**
     * Tests whether the class referenced by <code>owner</code> extends or
     * implements <code>superclass</code>. e.g.
     * isSuperClass("battlecode/common/GameActionException",
     * "java/lang/Throwable") => true
     *
     * @param owner
     *            - class to test
     * @param superclass
     *            - interface or superclass to test as an ancestor
     * @throws InstrumentationException
     *             if class <code>owner</code> cannot be found
     */
    private static boolean isSuperClass(String owner, String superclass) {
        ...
        try {
            ...
            ErrorReporter.report("Can't find the class \"" + owner
                    + "\", and this wasn't caught until the RobotMethodAdapter.isSuperClass stage.", true);
            throw new InstrumentationException();
        }
