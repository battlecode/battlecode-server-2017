...
public abstract class InstrumentingClassLoader extends ClassLoader {
    ...
    /**
     * Returns the value of the property bc.engine.lazy-instrumenter. If the
     * instrumenter is not lazy, then robots are killed if their class files
     * contain any forbidden code. If the instrumenter is lazy, then robots are
     * killed if they try to execute any forbidden code.
     * <p/>
     * Laziness might be necessary if we want to allow external libraries like
     * Guava or languages like Scala. If you're only using your own code then
     * it's better to turn laziness off so you don't get any bad surprises.
     * <p/>
     * {@see ClassReferenceUtil#illegalClass} {@see RoboMethodAdapter#forbidden}
     */
    ...
    /**
     * Returns the value of the property bc.engine.fast-hash. If fast-hash is
     * set, then the instrumenter will not use reflection to check whether an
     * object overrides hashCode(). The check is slow but without it, there is a
     * one in 2^32 chance of the instrumenter thinking that hashCode() is not
     * overridden when in fact it is overridden.
     * <p/>
    ...
    public byte[] instrument(String className, boolean checkDisallowed, String teamPackageName)
            throws InstrumentationException {
        // System.out.println("instrumenting "+className+", checkDisallowed
        // "+checkDisallowed);
        ClassReader cr;
        try {
        ...
        ClassWriter cw = new ClassWriter(COMPUTE_MAXS); // passing true sets
                                                        // maxLocals and
                                                        // maxStack, so we don't
                                                        // have to
        ClassVisitor cv = new RoboAdapter(cw, teamPackageName, debugMethodsEnabled, silenced, checkDisallowed);
        cr.accept(cv, 0); // passing false lets debug info be included in the
                          // transformation, so players get line numbers in
                          // stack traces
